...
import { getFillBusinessDetail, getFillLocationList, getSysDictionaryByTypeCode, addFillBusiness, editFillBusinessParameter, listBaseInfo } from '@/assets/js/API/api'
export default {
  name: 'addWaste',
  data () {
  ...
  methods: {
    getData () {
      ...
      getFillBusinessDetail(postData)
        .then(res => {
          if (this.checkObj.type === 'query' || this.checkObj.type === 'update') {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (_this.checkObj.type === 'update') {
            editFillBusinessParameter(postData)
              .then(res => {
    ...
    getSysDictionaryByTypeCodePreDeal () {
      ...
      getSysDictionaryByTypeCode(postData).then(res => {
