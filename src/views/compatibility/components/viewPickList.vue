    ...
    <div  class="bus-info-item">
      <el-tabs v-loading="pointLoading" v-model="defaultTabName" type="card" class="">
        <el-tab-pane v-for="item in recordList" :label="item.storageName" :name="item.storageId">
          ...
          <div class="item-header">
            <div class="item-header-text">领用明细</div>
          </div>
          <div class="item-body">
            <el-table size="mini" stripe border :header-cell-style="headClass" ref="multipleTable" :data="item.printDetailList" style="width:800px"
              height="250">
    ...
    methods: {
      ...
      getData() {
        ...
        let postData = {
          ...
          id: _this.checkObj ? _this.checkObj.compatibilityPlanId : _this.$route.query.compatibilityPlanId
        }
        listCompatibilityPrint(postData)
          .then(res => {
      ...
      submitDialog(formName) {
        ...
        var pos = curWwwPath.indexOf(pathname);
        var localhostPath = curWwwPath.substring(0, pos);
        window.open(localhostPath + pathname + '/#/printPickList?compatibilityPlanId=' + vm.checkObj.compatibilityPlanId+'&defaultTabName='+vm.defaultTabName);
      },
      cancelForm() {
