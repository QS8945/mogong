  ...
  <div class="bussiness-info">
    <el-form ref="form" :model="form" :rules="formRules" label-width="130px">
      <div class="bus-info-item">
        ...
        <div class="item-body">
          <el-row :gutter="100">
            <el-col :span="12">
              <el-form-item label="客户编号：" :prop="(checkObj.type==='query') ? '' : 'code'">
                <el-input v-model="form.code" :disabled="checkObj.type==='query'" placeholder="请输入客户编号"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
          ...
          <el-row :gutter="100">
            ...
            <el-col :span="12">
              <el-form-item label="企业种类：" :prop="(checkObj.type==='query') ? '' : 'type'">
                <el-select :disabled="checkObj.type==='query'" v-model="form.type" clearable filterable
                           placeholder="请选择企业种类">
                  <el-option
                          v-for="item in types"
                          :key="item.code"
          ...
          <el-row :gutter="100">
            <el-col :span="12">
              <el-form-item label="所属行政区：" :prop="(checkObj.type==='query') ? '' : 'cantonName'">
                <v-canton v-if="cantonVisible" :disabled="checkObj.type==='query'" :cantonName="form.cantonName" :cantonPlaceholder="'请选择行政区'" @changeCantonName="changeCantonNameP"></v-canton>
              </el-form-item>
            </el-col>
            <el-col :span="12">
          ...
          <el-row :gutter="100">
            <el-col :span="12">
              <el-form-item label="企业性质：" :prop="(checkObj.type==='query') ? '' : 'property'">
                ...
                <el-select v-model="form.property" :disabled="checkObj.type==='query'" clearable filterable
                           placeholder="请选择企业性质">
                  <el-option
          ...
          <el-row :gutter="100">
            <el-col :span="12">
              <el-form-item label="行业类型：" :prop="(checkObj.type==='query') ? '' : 'industryId'">
                <!--<el-input v-model="form.property"></el-input>-->
                <el-select v-model="form.industryId" :disabled="checkObj.type==='query'" clearable filterable
                           placeholder="请选择行业类型">
                  <el-option
              ...
              <el-form-item label="联系人：" :prop="(checkObj.type==='query') ? '' : 'contacts'">
                <el-row :gutter="20" class="init-row-margin">
                  <el-col :span="18" class="padding-left-none">
                    <el-input v-model="form.contacts" :disabled="checkObj.type==='query'" placeholder="请输入联系人"></el-input>
                  </el-col>
                  <el-col :span="6">
                    <el-button type="text" @click="contactManage" :disabled="checkObj.type==='query' || checkObj.type==='add'" class="el-btn-address">联系人管理</el-button>
                  </el-col>
                </el-row>
              </el-form-item>
            </el-col>
              ...
              <el-form-item label="客户负责人：" :prop="(checkObj.type==='query') ? '' : 'customerCharger'">
                <el-row :gutter="20" class="init-row-margin">
                  <el-col :span="18" class="padding-left-none">
                    ...
                    <el-select v-model="form.customerCharger" :disabled="checkObj.type==='query'" clearable filterable
                               placeholder="请选择客户负责人">
                      <el-option
                              v-for="item in customerChargers"
                              :key="item.id"
                              :label="item.name"
                              :value="item.id">
                      </el-option>
    ...
    <div class="view-total" v-if="contactsVisible">
      <el-dialog left :visible.sync="contactsVisible" :close-on-click-modal="false">
        <div slot="title" class="dialog-title">
          <div><span class="green-line"></span>联系人管理</div>
        </div>
        <v-contacts-management ref="contactsManagement" :entRelationId="form.entRelationId" @changeContactsVisible="changeContactsVisibleP" @optContact="optContactP" @reload="reloadContacts" @setCurrentContact="setCurrentContactP"></v-contacts-management>
        <el-dialog
                width="30%"
                title="内层 Dialog"
                :visible.sync="optContactsVisible"
                append-to-body>
                <div slot="title" class="dialog-title">
                  <div><span class="green-line"></span>新增联系人</div>
                </div>
          <v-contacts-add v-if="optContactsVisible" @reload="reloadP" :entRelationId="form.entRelationId" :editContactsData="editContactsData" :addCheckObj="addCheckObj" @changeAddContactsVisible="changeAddContactsVisibleP"></v-contacts-add>
        </el-dialog>
      </el-dialog>
    </div>
    <div class="view-total" v-if="historyCustomerVisible">
      <el-dialog left :visible.sync="historyCustomerVisible" :close-on-click-modal="false">
        <div slot="title" class="dialog-title">
          <div><span class="green-line"></span>历史负责人</div>
        </div>
...
import ContactsManagement from '@/views/customerManagement/dialog/contactsManagement'
import AddContactsManagement from '@/views/customerManagement/dialog/addContactsManagement'
import HistoryCustomer from '@/views/customerManagement/dialog/historyCustomer'
import Canton from '@/components/canton'
import { getSysCanton, addCustomerEnterprise, editCustomerEnterprise, getDispCodeAndNameDic, getIndustry, getEnterpriseInfo, getSysRoleEmploee } from '@/assets/js/API/api'
...
export default {
  ...
  created () {
    ...
    // (checkObj.type === 'update') || (this.form = checkObj.data)
  },
  components: {
            ...
            if (_this.checkObj.type === 'add') {
              addCustomerEnterprise(postData)
                .then(res => {
                  ...
                  let obj = {}
                  obj.type = 'update'
                  obj.data = res.data.entRelationId
                  obj.activeName = 'companyInfo'
                  this.$router.push({
              ...
              editCustomerEnterprise(postData)
                .then(res => {
