    ...
    <div class="form-format contacts-management">
        <div class="card-list">
          <el-card class="box-card" v-for="item in dataList" :key="item.contactName" :class="{'current-contact': item.id === contactId}">
            <div slot="header" class="clearfix link-point" @click="setCustomerContactsPrincipal(item)">
              <span>{{(item.id === contactId) ? '当前' : ''}}联系人</span>
              <span class="btn-group">
                <el-button plain type="text"><i class="el-icon-delete" @click.stop="optContact('del', item.id)"></i></el-button>
                <el-button plain type="text"><i class="el-icon-edit" @click.stop="optContact('update', item)"></i></el-button>
              </span>
              <!--<el-button style="float: right; padding: 3px 0" type="text">操作按钮</el-button>-->
            </div>
            <div class="text item link-point" @click="setCustomerContactsPrincipal(item)">
              ...
              <el-tooltip :open-delay="700" :content="'备注:'+item.remark" placement="top" :enterable="false">
                <div class="item-body">
                  <span class="item-text">备注:</span>{{item.remark}}
                </div>
              </el-tooltip>
            </div>
          </el-card>
...
import { getCodeAndNameDic, getClassCode, getWasteCode, getCustomerContacts, deleteContacts, setCustomerContactsPrincipal } from '@/assets/js/API/api'
export default {
  name: 'addWaste',
  data () {
    return {
  ...
  methods: {
    ...
    getData () {
      let _this = this
      let postData = {
        entRelationId: this.entRelationId
      }
      getCustomerContacts(postData)
        .then(res => {
      ...
      switch (type) {
        ...
        case 'del':
          ...
          this.$confirm('确认删除？', '提示', {
            ...
            deleteContacts(postData).then(res => {
